...
Built for the **1M1B AI for Sustainability Virtual Internship** (with IBM SkillsBuild and AICTE).
...
## How it works (RAG)
1. Rules text (`rules.txt`) is split into clauses.
2. A question is matched to the most relevant clauses (TF-IDF retrieval).
3. The matched clauses are shown as sources.
4. Optional: IBM Granite answers using only those clauses and cites them.
...
## Author
Prerna - BSc IT, Mumbai University
